...
def doc_type(array, col1, col2)
  if array.length == 2
    search_all(@start.midnight, @end.end_of_day, theme(@theme),source(@source), col1, col2)
  elsif array.include?("主文") 
    search_post(@start.midnight, @end.end_of_day, theme(@theme),source(@source), col1, col2)
  else 
    search_comment(@start.midnight, @end.end_of_day, theme(@theme),source(@source), col1, col2)
  end 
end 
...
def search_all(start_date,end_date,keywords,source, col1, col2)
  count = search_post(@start.midnight, @end.end_of_day, theme(@theme),source(@source), col1, col2)[1]+search_comment(@start.midnight, @end.end_of_day, theme(@theme),source(@source), col1, col2)[1]
  result = search_post(@start.midnight, @end.end_of_day, theme(@theme),source(@source), col1, col2)[0]|search_comment(@start.midnight, @end.end_of_day, theme(@theme),source(@source), col1, col2)[0]
  return result, count 
end 
