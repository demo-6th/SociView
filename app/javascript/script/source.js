document.addEventListener("turbolinks:load", () => {
    ...
    const ptt_divsion = Object.values(ptt_line)
        .map(function(n, i) {
            return n / Object.values(total_result)[i];
        })
        .map((x) => Math.round(x * 10000) / 100 || 0);
    const dcard_divsion = Object.values(dcard_line)
        .map(function(n, i) {
            return n / Object.values(total_result)[i];
        })
        .map((x) => Math.round(x * 10000) / 100 || 0);
    ...
    const dcard_source1_divsion = Object.values(dcard_source1)
        .map(function(n, i) {
            return n / Object.values(dcard_result)[i];
        })
        .map((x) => Math.round(x * 10000) / 100 || 0);
    const dcard_source2_divsion = Object.values(dcard_source2)
        .map(function(n, i) {
            return n / Object.values(dcard_result)[i];
        })
        .map((x) => Math.round(x * 10000) / 100 || 0);
    const dcard_source3_divsion = Object.values(dcard_source3)
        .map(function(n, i) {
            return n / Object.values(dcard_result)[i];
        })
        .map((x) => Math.round(x * 10000) / 100 || 0);
    const dcard_source4_divsion = Object.values(dcard_source4)
        .map(function(n, i) {
            return n / Object.values(dcard_result)[i];
        })
        .map((x) => Math.round(x * 10000) / 100 || 0);
    const dcard_source5_divsion = Object.values(dcard_source5)
        .map(function(n, i) {
            return n / Object.values(dcard_result)[i];
        })
        .map((x) => Math.round(x * 10000) / 100 || 0);
    const dcard_source1_bgc = dcard_source1_divsion.map(
        (x) => (x = "rgba(244,143,177,0.7)")
    );
    const dcard_source2_bgc = dcard_source2_divsion.map(
        (x) => (x = "rgba(129,212,250,0.7)")
    );
    const dcard_source3_bgc = dcard_source3_divsion.map(
        (x) => (x = "rgba(178,151,196,0.7)")
    );
    const dcard_source4_bgc = dcard_source4_divsion.map(
        (x) => (x = "rgba(245,203,116,0.7)")
    );
    const dcard_source5_bgc = dcard_source5_divsion.map(
        (x) => (x = "rgba(55,71,79,0.7)")
    );

    const ptt_source1_divsion = Object.values(ptt_source1)
        .map(function(n, i) {
            return n / Object.values(ptt_result)[i];
        })
        .map((x) => Math.round(x * 10000) / 100 || 0);
    const ptt_source2_divsion = Object.values(ptt_source2)
        .map(function(n, i) {
            return n / Object.values(ptt_result)[i];
        })
        .map((x) => Math.round(x * 10000) / 100 || 0);
    const ptt_source3_divsion = Object.values(ptt_source3)
        .map(function(n, i) {
            return n / Object.values(ptt_result)[i];
        })
        .map((x) => Math.round(x * 10000) / 100 || 0);
    const ptt_source4_divsion = Object.values(ptt_source4)
        .map(function(n, i) {
            return n / Object.values(ptt_result)[i];
        })
        .map((x) => Math.round(x * 10000) / 100 || 0);
    const ptt_source5_divsion = Object.values(ptt_source5)
        .map(function(n, i) {
            return n / Object.values(ptt_result)[i];
        })
        .map((x) => Math.round(x * 10000) / 100 || 0);
    const ptt_source1_bgc = ptt_source1_divsion.map(
        (x) => (x = "rgba(255,0,0,0.5)")
    );
    const ptt_source2_bgc = ptt_source2_divsion.map(
        (x) => (x = "rgba(255,255,0,0.5)")
    );
    const ptt_source3_bgc = ptt_source3_divsion.map(
        (x) => (x = "rgba(0,255,0,0.5)")
    );
    const ptt_source4_bgc = ptt_source4_divsion.map(
        (x) => (x = "rgba(0,0,0,0.5)")
    );
    const ptt_source5_bgc = ptt_source5_divsion.map(
        (x) => (x = "rgba(255,0,255,0.5)")
    );

    if (document.getElementById("sourceBarChart")) {
        // source
        const ctx_bar = document.getElementById("sourceBarChart").getContext("2d");
